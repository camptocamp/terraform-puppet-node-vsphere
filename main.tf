...
resource "vsphere_virtual_machine" "puppet-node" {
  count            = "${var.instance_count}"
...
resource "puppetdb_node" "puppet-node" {
  count    = "${var.instance_count}"
  certname = "${format("ip-%s.%s", join("-", split(".", cidrhost(var.iprange, var.hostnum + count.index))), var.domain)}"
}

resource "puppetca_certificate" "puppet-node" {
  count = "${var.instance_count}"
  name  = "${format("ip-%s.%s", join("-", split(".", cidrhost(var.iprange, var.hostnum + count.index))), var.domain)}"
}
